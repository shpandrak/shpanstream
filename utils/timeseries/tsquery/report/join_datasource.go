...
func (mds JoinDatasource) Execute(ctx context.Context, from time.Time, to time.Time) (Result, error) {
	...
	if err != nil {
		return util.DefaultValue[Result](),
	...
	for idx, ds := range downStreamResults {
		...
		for fIdx := range currFieldsMeta {
			if b := fieldUrnsSet[currFieldsMeta[fIdx].Urn()]; b {
				return util.DefaultValue[Result](), fmt.Errorf("duplicate field urn %s found while joining datasources", currFieldsMeta[fIdx].Urn())
			}
		}
	}
	...
	default:
		return util.DefaultValue[Result](), fmt.Errorf("unsupported join type %d", mds.joinType)
	}
	return NewResult(
		joinedFieldMeta,
		joinedStreams,
