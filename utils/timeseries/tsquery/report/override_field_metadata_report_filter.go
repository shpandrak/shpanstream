...
func (ofm OverrideFieldMetadataFilter) Filter(ctx context.Context, result Result) (Result, error) {
	...
	if fieldIndex == -1 {
		return util.DefaultValue[Result](), fmt.Errorf("field with urn %s not found in result", ofm.fieldUrn)
	}
	...
	if ofm.optUpdatedUrn != nil {
		if *ofm.optUpdatedUrn != originalFieldMeta.Urn() {
			...
			if result.HasField(*ofm.optUpdatedUrn) {
				return util.DefaultValue[Result](), fmt.Errorf("cannot update field URN to %s, since it already exists in the result", *ofm.optUpdatedUrn)
			}
	...
	return NewResult(newFieldsMeta, result.Stream()), nil
}
