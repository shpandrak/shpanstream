...
func (te TimestampExtractFieldValue) Execute(ctx context.Context, fieldsMeta []tsquery.FieldMeta) (tsquery.ValueMeta, ValueSupplier, error) {
	if te.location == nil {
		return util.DefaultValue[tsquery.ValueMeta](), nil, fmt.Errorf("timestampExtract location must not be nil")
	}

	sourceMeta, sourceSupplier, err := te.source.Execute(ctx, fieldsMeta)
	if err != nil {
