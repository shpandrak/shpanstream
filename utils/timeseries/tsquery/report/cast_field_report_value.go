...
func (cf CastFieldValue) Execute(ctx context.Context, fieldsMeta []tsquery.FieldMeta) (tsquery.ValueMeta, ValueSupplier, error) {
	// Execute source to get metadata (lazy validation)
	sourceMeta, sourceValueSupplier, err := cf.source.Execute(ctx, fieldsMeta)
	if err != nil {
