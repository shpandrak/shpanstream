...
func TestRateFilter_NonNegative(t *testing.T) {
	...
	// Rate 2: decrease detected (50 < 200), delta clamped to 0, rate = 0
	require.InDelta(t, 0.0, records[1].Value.(float64), 1e-10)
	require.Equal(t, baseTime.Add(2*time.Hour), records[1].Timestamp)
...
func TestRateFilter_NonNegative_EmitOnReset(t *testing.T) {
	ctx := context.Background()
	baseTime := time.Date(2025, 1, 1, 0, 0, 0, 0, time.UTC)
	timestamps := []time.Time{
		baseTime,
	...
	// Rate 2: reset detected with emitOnReset, delta = 50 (current value), rate = 50/3600
