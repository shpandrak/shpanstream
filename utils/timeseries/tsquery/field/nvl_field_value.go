...
func (nf NvlFieldValue) Execute(fieldsMeta []tsquery.FieldMeta) (ValueMeta, ValueSupplier, error) {
	...
	if err != nil {
		return util.DefaultValue[ValueMeta](), nil, fmt.Errorf("failed executing alternative field: %w", err)
	}
	...
	if !altMeta.Required {
		return util.DefaultValue[ValueMeta](), nil, fmt.Errorf("alternative field must be required")
	}
