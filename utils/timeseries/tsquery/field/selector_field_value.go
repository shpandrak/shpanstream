...
func (cf SelectorFieldValue) Execute(fieldsMeta []tsquery.FieldMeta) (ValueMeta, ValueSupplier, error) {
	...
	if err != nil {
		return util.DefaultValue[ValueMeta](), nil, fmt.Errorf("failed executing selector boolean field: %w", err)
	}

	trueMeta, trueValueSupplier, err := cf.trueField.Execute(fieldsMeta)
	if err != nil {
		return util.DefaultValue[ValueMeta](), nil, fmt.Errorf("failed executing true field: %w", err)
	}

	falseMeta, falseValueSupplier, err := cf.falseField.Execute(fieldsMeta)
	if err != nil {
		return util.DefaultValue[ValueMeta](), nil, fmt.Errorf("failed executing false field: %w", err)
	}
