...
func (ufv UnaryNumericOperatorFieldValue) Execute(ctx context.Context, fieldMeta tsquery.FieldMeta) (tsquery.ValueMeta, ValueSupplier, error) {
	// Execute operand to get metadata (lazy validation)
	operandMeta, operandValueSupplier, err := ufv.operand.Execute(ctx, fieldMeta)
	if err != nil {
