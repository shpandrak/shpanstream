...
func (nf NvlFieldValue) Execute(ctx context.Context, fieldMeta tsquery.FieldMeta) (tsquery.ValueMeta, ValueSupplier, error) {
	// Execute both fields to get metadata (lazy validation)
	sourceMeta, sourceValueSupplier, err := nf.source.Execute(ctx, fieldMeta)
	if err != nil {
		return util.DefaultValue[tsquery.ValueMeta](), nil, fmt.Errorf("failed executing source field: %w", err)
	}
	altMeta, altValueSupplier, err := nf.altField.Execute(ctx, fieldMeta)
	if err != nil {
