...
func (a DropFieldsFilter) Filter(result tsquery.Result) (tsquery.Result, error) {
	...
	if len(fieldIdxToKeep) == 0 {
		return util.DefaultValue[tsquery.Result](), fmt.Errorf("cannot drop all fields from the result")
	}
	if countFoundToDrop != len(a.fieldUrnsSetToRemove) {
		...
		return util.DefaultValue[tsquery.Result](), fmt.Errorf("cannot drop fields, since they do not exist: %v", missingFields)
	}
