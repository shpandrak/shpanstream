...
*Requested by shpankids, a consumer pinned at `v0.4.57` that runs on Cloud Firestore. Checked
...
three small adapters that shpankids wrote for itself and that any Firestore user of shpanstream
needs. The root module stays dependency-free.

## What moves, and why it's generic

**1. Query to stream.** Source: shpankids `infra/database/firestore/firestore_stream_provider.go`,
55 LOC.
...
**2. Document refs to stream.** Source: shpankids `firestore_doc_id_stream_provider.go`, 43 LOC.
...
**3. Awaited bulk write.** Source: shpankids `firestore_bulk_writer.go`, 46 LOC, which fixes a
silent failure.
...
So code that enqueues, flushes and returns `nil` reports success for writes the server refused.
shpankids found this on a one-shot migration: its ledger recorded "applied" over data that was
...
The transaction resolver matters. shpankids carries its transaction in the context, and its
...
2. With a resolver that returns a transaction, the read goes through the transaction.
   Consuming the stream *after* a write in the same transaction fails with the client's
   read-after-write error. That only happens if the query really went through `tx`.
   Without a resolver, the same sequence succeeds.
...
- Replace its two providers and `awaitBulkWrites` with the module, passing its own context-key
  lookup as the resolver.
- Keep its document-format layer (`firestore_format.go`: native timestamps and integer handling)
  in shpankids, as the `mapper` / `enqueue` functions. That layer is specific to the app.
- Net effect: about 100 LOC fewer in shpankids.
...
- Priority: low for shpankids, which gains about 100 LOC. It is the only shpankids infrastructure
  that clearly fits shpanstream's scope. The rest of the shpankids storage layer, a JSON document
  store with an in-memory Firestore stand-in, is a store abstraction rather than a stream library,
