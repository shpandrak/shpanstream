...
*Requested by shpankids, a consumer pinned at `v0.4.57`. Reproduced on master `bc9c340` on
2026-09-18.*

## Ask

Every operator that keeps per-consumption state should reset it when the stream is opened. Then
a stream value behaves the same way however many times it is consumed, and however far each
earlier consumption got.
...
shpankids uses `FromLazy` in 10 places and `Limit` in two live places, one of them the activity
feed (`internal/api/oapi_shpankids_my_impl.go:72`). Today each of those streams happens to be
consumed exactly once, so nothing is broken yet. The trap is Firestore transactions:
- Firestore reruns a transaction's closure when there is contention.
- shpankids opens streams with the transaction carried in the context of the consumer.
...
The code reviewed so far builds its streams inside the closure, but only by convention; nothing
enforces it. Operators that reset on `Open` would remove the trap entirely.
...
Nothing at the call sites: every current use keeps working and the trap disappears. Two sites that
wrap a lazily loaded value into a stream stay on `FromLazy` and become safe to consume again:
...
- Priority: high as a library correctness fix. For shpankids today it is a latent trap, not an
  active bug.
