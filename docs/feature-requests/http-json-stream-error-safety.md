...
*Filed by a consumer in production, pinned at `v0.4.57`, that uses the strict-interface oapi
...
**The size of the problem.** The consumer serves 27 array endpoints through this template. Most of
...
- a per-element mapping error, such as a stored document that won't decode, or a per-row lookup
  inside `MapWithErrAndCtx`;
- a Firestore iterator error in the middle of a long export stream.

**The consequences.** When one does happen:
- the client sees a JSON parse failure instead of the server's error;
- request metrics record a success;
- the only trace is the error handler's log line.

**How change 1 alone helps.** Buffering alone makes almost every one of this consumer's responses
all-or-nothing, because almost all of them are well under 32 KiB.
...
- **The library never panics on its own.** Consumers often wrap handlers in a recovery middleware
  that would swallow `http.ErrAbortHandler` (this consumer's does) and then append a JSON error
  body, which is the same corruption. Returning a recognisable error leaves the abort to the
  consumer, who knows their middleware stack.
...
1. Bump the pin: `go.mod` together with the template URL in its oapi-codegen config.
2. In its response error handler, on `ResponseCommittedError`: log the error, then
   `panic(http.ErrAbortHandler)`.
3. Change its recovery middleware to re-panic `http.ErrAbortHandler` instead of writing a body.

## Workaround today

This can be done without the library. The consumer could add a middleware that:
- wraps `http.ResponseWriter` to record whether `WriteHeader` or `Write` has run;
- makes the error handler abort instead of calling `http.Error` when it has.
